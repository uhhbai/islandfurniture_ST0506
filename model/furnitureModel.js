    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "SELECT i.ID as id, i.NAME as name, f.IMAGEURL as imageURL, i.SKU as sku, i.DESCRIPTION as description," +
...
var sql = "SELECT i.ID as id, i.NAME as name, f.IMAGEURL as imageURL, i.SKU as sku, i.DESCRIPTION as description," +
          ...
          " IFNULL(p.DISCOUNTRATE, 0) as discount" + // <--- Get discount or 0
          " FROM itementity i" +
          " JOIN furnitureentity f ON i.ID = f.ID" +
          " JOIN item_countryentity ic ON i.ID = ic.ITEM_ID" +
          " LEFT JOIN promotionentity p ON i.ID = p.ITEM_ID" + // <--- Left Join allows items with no promo
          " WHERE i.ISDELETED=FALSE AND ic.COUNTRY_ID=? AND i.CATEGORY=?;";
            conn.query(sql, [countryId, cat], function (err, result) {
              if (err) {
                ...
                for (var i = 0; i < result.length; i++) {
                  ...
                  fur.discount = result[i].discount; // <--- Set discount value
                  furList.push(fur);
                }
                conn.end();
                return resolve(furList);
        ...
        if (err) {
          ...
          if (countryId == null || countryId == "") {
            ...
            var sql =
              ...
              "i.TYPE as type, i._LENGTH as length, i.WIDTH as width, i.HEIGHT as height, i.CATEGORY as category, " +
              "ic.RETAILPRICE as price, " +
              "IFNULL(p.DISCOUNTRATE, 0) as discount " + // <--- The new discount logic
              "FROM itementity i " +
              "JOIN furnitureentity f ON i.ID = f.ID " +
              "JOIN item_countryentity ic ON i.ID = ic.ITEM_ID " +
              "LEFT JOIN promotionentity p ON i.ID = p.ITEM_ID " +
              "WHERE i.ISDELETED = FALSE AND ic.COUNTRY_ID = ? AND i.SKU = ?;";
            conn.query(sql, [countryId, sku], function (err, result) {
              if (err) {
                ...
                fur.discount = result[0].discount; // <--- Set the discount value
                conn.end();
                return resolve(fur);
              }
            });
    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "INSERT INTO itementity(DTYPE,SKU,_LENGTH,CATEGORY,DESCRIPTION,HEIGHT,NAME,TYPE,VOLUME,WIDTH)" +
            "values(?,?,?,?,?,?,?,?,?,?)";
          conn.query(sql, sqlArgs, function (err, result) {
            if (err) {
              ...
              if (result.affectedRows > 0) {
                furnitureDB
                  .addFurniture2(result.insertId, data.imgPath)
                  .then((result) => {
                    conn.end();
    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "UPDATE itementity SET NAME=?, CATEGORY=?, DESCRIPTION=? WHERE ID=?";
          conn.query(
            sql,
    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "DELETE FROM furnitureentity WHERE ID IN (" + idString + ")";
          conn.query(sql, function (err, result) {
            if (err) {
              conn.end();
              return reject(err);
    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "DELETE FROM item_countryentity WHERE ITEM_ID IN (" +
            idString +
            ")";
          conn.query(sql, function (err, result) {
            if (err) {
