    ...
    return new Promise((resolve, reject) => {
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          var sql =
            "SELECT i.SKU, i.NAME AS 'ITEM_NAME', ic.RETAILPRICE, li.QUANTITY, sr.CREATEDDATE, " +
            "f.IMAGEURL, sr.ID, d.NAME, d.DELIVERY_ADDRESS, d.POSTAL_CODE, d.CONTACT " +
            "FROM salesrecordentity sr " +
            "JOIN salesrecordentity_lineitementity sl ON sr.ID = sl.SalesRecordEntity_ID " +
            "JOIN lineitementity li ON sl.itemsPurchased_ID = li.ID " +
            "JOIN itementity i ON li.ITEM_ID = i.ID " +
            "JOIN item_countryentity ic ON ic.ITEM_ID = i.ID AND ic.COUNTRY_ID = 25 " +
            "JOIN furnitureentity f ON f.ID = i.ID " +
            "LEFT JOIN deliverydetailsentity d ON d.SALERECORD_ID = sr.ID " +
            "WHERE sr.MEMBER_ID = ? " +
      ...
      conn.connect(function (err) {
        if (err) {
          ...
          if (password == null || password == "") {
            ...
            // ✅ Optional: log query params here too
            console.log("SQL args (no password):", sqlArgs);
